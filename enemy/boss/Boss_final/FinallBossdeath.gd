...
func enter():
	...
	# Сохраняем ссылки ДО queue_free ← вот главный фикс
	...
	# Теперь используем сохранённые ссылки ← а не enemy.get_tree()
	await scene_tree.create_timer(1.0).timeout
	_show_good_ending(scene_tree, current_scene)

func _show_good_ending(scene_tree: SceneTree, current_scene: Node):
	...
	var stats = Label.new()
	stats.text = "⭐ EXP: " + str(GameState.exp) + "   ⚔️ Уровень меча: " + str(GameState.sword_level + 1)
