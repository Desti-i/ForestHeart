	...
	match GameState.quest_vampire:
		...
		GameState.QuestState.COMPLETED:
			dialog_lines = [
				...
				"Но я должен тебе предупредить, чтобы догнать злодея",
				"нужно убить несколько боссов на локациях.",
				"У первого босса гоблинов есть иммунитет к атакам мечом",
				"Советую купить и прокачать магию огня и получить магию лечения",
				"Иди и стань сильнее!"
			]
